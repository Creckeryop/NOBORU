...
ParserManager = {
    ...
    getMangaListAsync = function (i, table, index)
        if parser == nil or (task ~= nil and task.table == table and task.index == index) then return end
        ...
        table[index] = {}
        local new_task = {type = 'MangaList', f = function() parser:getManga(i, table, index) end, table = table, index = index}
        order_count = order_count + 1
        order[order_count] = new_task
    end,
    getChaptersAsync = function (manga)
        if parser == nil or (task ~= nil and task.table == manga and task.index == 'chapters') then return end
        for _,v in ipairs(order) do
        ...
        manga.chapters = {}
        local new_task = {type = 'Chapters', f = function() parser:getChapters(manga, 'chapters') end, table = manga, index = 'chapters'}
        order_count = order_count + 1
        order[order_count] = new_task
    end,
    getChapterInfoAsync = function (chapter)
        if parser == nil or (task ~= nil and task.table == chapter and task.index == 'pages') then return end
        for _,v in ipairs(order) do
        ...
        chapter.pages = {}
        local new_task = {type = 'Info', f = function() parser:getChapterInfo(chapter, 'pages') end, table = chapter, index = 'pages'}
        order_count = order_count + 1
        order[order_count] = new_task
    end,
    updateParserList = function ()
        local new_task = {type = 'Update', f = function () 
