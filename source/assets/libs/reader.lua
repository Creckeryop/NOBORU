...
Reader = {
    draw = function()
        if STATE == STATE_LOADING then
            ...
            Font.print(LUA_FONT, 480 - width2 / 2, 272 - 10, loadingManga_name, LUA_COLOR_BLACK)
        elseif STATE == STATE_READING then
            for i = -1, 1 do
                    ...
                    if (type(page.image.e or page.image) == "table") then
                        for k = 1, page.image.parts do
                            if page.image[k] and page.image[k].e ~= nil then
                                ...
                                local width = Font.getTextWidth(LUA_FONT, loading)
                                Font.print(LUA_FONT, offset.x + 960 * i + 480 - width / 2, offset.y + page.y + (k - 1) * page.image.part_h * page.zoom - page.height / 2 * page.zoom + 10 * page.zoom, loading, LUA_COLOR_BLACK)
                            end
                        end
                    else
                ...
                elseif page ~= nil then
                    ...
                    local width = Font.getTextWidth(LUA_FONT, loading)
                    Font.print(LUA_FONT, offset.x + 960 * i + 480 - width / 2, 272 - 10, loading, LUA_COLOR_BLACK)
                end
            end
            if Pages.page <= Pages.count and Pages.page > 0 then
                local width = Font.getTextWidth(LUA_FONT, Pages.page .. "/" .. Pages.count) + 20
                local height = Font.getTextHeight(LUA_FONT, Pages.page .. "/" .. Pages.count)
                Graphics.fillRect(960 - width, 960, 0, height + 5, Color.new(0, 0, 0, 128))
                Font.print(LUA_FONT, 960 - width + 10, 0, Pages.page .. "/" .. Pages.count, LUA_COLOR_WHITE)
            end
        end
    end,
    update = function()
                    ...
                    if PageChanged~=nil then
                        if PageChanged ~= Pages.page and Pages[PageChanged] and Pages[PageChanged].image and type(Pages[PageChanged].image.e or Pages[PageChanged].image) =="table" then
                            ...
                            if math.abs(PageChanged-Pages.page)==1 and Pages[PageChanged].link ~= "loadnext" and Pages[PageChanged].link ~= "loadprev" then
                                Net.downloadImageAsync(Pages[PageChanged].link, Pages[PageChanged],'image')
                            end
                        end
                        PageChanged = nil
                    end
